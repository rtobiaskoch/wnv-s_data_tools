gsheet_pull = function(key, sheet, out_fn, update = T) {
  ...
  if (update == T) {
    ...
    # if you don't want to pull it off google drive load the out_fn locally
    if (file.exists(out_fn)) {
      read.csv(out_fn)
    } else {
      print(paste(
        out_fn,
        "file doesn't exist, please set update = T or download desired gsheet file"
      ))
    }
  }
}
