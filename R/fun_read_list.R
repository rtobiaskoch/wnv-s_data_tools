...
read_list <- function(path, pattern) {
  ...
  summary_lines <- pmap_chr(
    list(file_name, rows, cols),
    ~ paste(..., sep = "\t")
  )
  ...
  cat(
    "\n\nFinal combined data has the following dimensions:",
    dim(combined_df),
    "\n"
  )
  ...
  if (ncol(combined_df) != round(mean(cols), 2)) {
    cat(
      "\nWarning number of columns in combined_df ",
      ncol(combined_df),
      "and average in df_list",
      mean(cols),
      "don't match.\n"
    )
  }
