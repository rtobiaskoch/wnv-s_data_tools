...
wnv_s_clean <- function(df, 
  ...
  if ("spp" %in% names(df) & "trap_status" %in% col_2_clean) {
    ...
    # Guard: ensure trap_status column exists so the any() check below is valid
    # on a first pass where the column hasn't been created yet.
    if (!"trap_status" %in% names(df)) {
      df <- df %>% mutate(trap_status = NA_character_)
    }

    # "malfunction" is set from raw spp on the first pass, then spp is cleaned
    # to "none" — indistinguishable from "no mosquitoes". The any() check on the
    # existing column re-signals malfunction before case_when overwrites it.
    # Only malfunction needs this treatment; "no mosquitoes" must be allowed to
    # be recomputed so it doesn't persist and inflate counts when a database row
