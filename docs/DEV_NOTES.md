  ...
  └─ sources:
     ...
     [ per_source_clean chunk ]
        ...
        clean_one(df_raw, src, src_name)
              ...
              │     status events (malfunction, no_culex, no_mosquitoes) get total=NA or 0
              └─ filter_culex_sheet()  ← skipped when src$filter_active = FALSE (e.g. Boulder)
                    semi-joins against foco_trap active-trap reference
              │
              ▼
